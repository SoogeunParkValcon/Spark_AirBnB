...
# MAGIC # Feature Store
# MAGIC
# MAGIC The <a href="https://docs.databricks.com/applications/machine-learning/feature-store.html" target="_blank">Databricks Feature Store</a> is a centralized repository of features. 
# MAGIC
...
import mlflow
import mlflow.sklearn # we will use sklearn to build models: Feature Store is not just specific for Spark
...
# MAGIC
# MAGIC > We will append an ```index``` column. This is different from traditional Spark tables and Delta Lake table, which already come with the primary key...
...
# load data
Df = spark.read.format("delta").load(f"{absolute_dir_path}/imputed_results")



# COMMAND ----------

airbnb_df = Df.withColumn("index", monotonically_increasing_id())
...
random_schema_name = "soogeuniejjang"

# COMMAND ----------



spark.sql(f"CREATE DATABASE IF NOT EXISTS {random_schema_name}")
table_name = f"{random_schema_name}.airbnb_hiii"
print(table_name)
...
# making new dataframe only with the numeric columns
...
fs.create_table(
    ...
    schema=numeric_features_df.schema, # simply just the schema of the dataframe that we just created
    description="Numeric features of airbnb data. I exclude the target column price." # simple string that describes
)
...
# MAGIC
# MAGIC >> Getting these information programmatically!!
...
# making a new column: score_difference from last month! (simulated data)
...
def load_data(table_name, lookup_key):
    ...
    # excluding index because that's just row numbers
...
def train_model(X_train, X_test, y_train, y_test, training_set, fs):
    ...
    with mlflow.start_run() as run:
        ...
        # logging the model things with the Feature Store here!
        fs.log_model(
            model=rf,
...
batch_input_df = inference_data_df.drop("price") # Exclude true target label

predictions_df = fs.score_batch(f"models:/{model_name}/1", 
                                  batch_input_df, result_type="double")

# scoring method in the feature store...??
# -> it takes a model, and input_df, and then return the result. Like the transform method of pipeline I guess?
...
# making the average review score column, and then removing the other review scores
...
def train_model(X_train, X_test, y_train, y_test, training_set, fs):
    ...
    with mlflow.start_run() as run:
        ...
        # logging the model in the feature store again
        fs.log_model(
            model=rf,
            artifact_path="feature-store-model",
            flavor=mlflow.sklearn,
