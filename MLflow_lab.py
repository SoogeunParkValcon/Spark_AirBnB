...
# MAGIC
# MAGIC
# MAGIC # Amsterdam Airbnb: MLflow Lab
# MAGIC
...
# this is a delta lake format. And by default, this is the default..
...
# I wanto try and run this again, but without making the "run" go under the registered model:

with mlflow.start_run(run_name="hallo_model") as run:
    # Log parameters
    mlflow.log_param("label", "log-price")
    mlflow.log_param("data_version", data_version)
    mlflow.log_param("data_path", train_delta_path)    
...
display(mlflow.search_runs(filter_string = f"params.data_version = {data_version}"))
