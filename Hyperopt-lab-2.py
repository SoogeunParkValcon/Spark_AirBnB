...
# load data
spark_df = spark.read.format("delta").load(f"{absolute_dir_path}/imputed_results")
...
# split 80/20 train-test, using train_test_split from sklearn.model_selection. 
# this is how the split is done for pandas dataframes
...
def objective_function(params):
    ...
    # Evaluate predictions via cross-validation
...
# MAGIC
# MAGIC Instead of using the default **`Trials`** class, you can leverage the **`SparkTrials`** class to trigger the distribution of tuning tasks across Spark executors. 
# MAGIC
# MAGIC >> On Databricks, SparkTrials are automatically logged with MLflow.
# MAGIC
...
with mlflow.start_run(run_name="best_model"):
    ...
    regressor = RandomForestRegressor(max_depth=best_max_depth, 
