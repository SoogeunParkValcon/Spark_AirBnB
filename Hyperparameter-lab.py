...
airbnb_df = (spark
            .read
            .format("delta")
            .load(f"{absolute_dir_path}/imputed_results")
            .withColumn("priceClass", (col("price") >= 150).cast("int"))
            .drop("price")
           )
# .withColumn() is the one that is being created: binary outcome
...
# excluding the priceClass from the input features
...
# MAGIC
# MAGIC >> Apaprently, Area under ROC can be interpreted similar to R^2. This is between 0 and 1, just like R^2.
...
# I want to view the stages of this pipeline_model
print(pipeline_model.stages[-1])

print(pipeline_model.stages[2])
...
# so although the pipeline_model includes the CV in the pipeline, I guess this just selects the best model and applies it
