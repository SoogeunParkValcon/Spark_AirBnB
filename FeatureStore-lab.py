...
# load data
airbnb_df = spark.read.format("delta").load(f"{absolute_dir_path}/imputed_results").withColumn("index", monotonically_increasing_id())
...
schema_name = "soogeunparkschema"

# COMMAND ----------

spark.sql(f"CREATE DATABASE IF NOT EXISTS {schema_name}")
table_name = f"{schema_name}.airbnb_df"
...
# MAGIC
# MAGIC In this simple example we want to predict the price with other features.
# MAGIC
# MAGIC Fill in the feature computation function below to select only the feature columns, not **`price`**.

# COMMAND ----------

@feature_store.feature_table
def select_features(dataframe, columns):
...
airbnb_features_df = select_features(airbnb_df, columns = columns)
display(airbnb_features_df.limit(5))
...
fs.create_table(
    ...
    description= "airbnb_df in a feature store by Soogeun"
)
...
target_df = airbnb_df.select(["index", "price"])
...
def load_data(table_name, lookup_key):
    ...
    training_set = fs.create_training_set(target_df, model_feature_lookups, label="price", exclude_columns=["index"])
